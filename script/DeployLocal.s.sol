...
contract DeployLocal is Script {
    ...
    function run() external {
        ...
        (address predicted, bytes32 salt) = HookMiner.find(
            CREATE2_DEPLOYER, FLAGS, type(PowerPerpHook).creationCode, abi.encode(IPoolManager(address(manager)), 8_000, 30, "Power Perp LP", "PP-LP")
        );
        hook = new PowerPerpHook{salt: salt}(IPoolManager(address(manager)), 8_000, 30, "Power Perp LP", "PP-LP");
