...
/**
 * @title DeployPowerPerp
 * @notice Deploys PowerPerp deterministically to any chain with a Uniswap v4 `PoolManager`.
 *
 ...
 */
contract DeployPowerPerp is Script {
    ...
    function run() external {
        ...
        bytes memory creationCode = type(PowerPerpHook).creationCode;
        bytes memory constructorArgs = abi.encode(manager, _weight0Bps, _swapFeeBps, shareName, shareSymbol);
        ...
        console2.log("PowerPerp ", predicted);
        ...
        vm.startBroadcast();
        PowerPerpHook hook = new PowerPerpHook{salt: salt}(manager, _weight0Bps, _swapFeeBps, shareName, shareSymbol);
        vm.stopBroadcast();
        ...
        require(
            keccak256(bytes(IHookMetadata(address(hook)).hookName())) == keccak256(bytes("PowerPerp")),
