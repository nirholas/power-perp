# Integrating PowerPerp

## From Solidity

```solidity
import {PowerPerpHook} from "power-perp/src/hooks/PowerPerpHook.sol";
...
const hook = getHook("power-perp");
const address = hookAddress("power-perp", 8453);              // Base
...
```solidity
IHookMetadata(hookAddress).hookName();   // "PowerPerp"
...
More at https://power-perp.pages.dev.
